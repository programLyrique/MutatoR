...
- **Compiled code is built once, not per mutant.** For the `testthat` strategy,
   ...
   never mutated.  The `installed`strategy compiles once into a _template_ library and
...
Coverage guidance applies to the `testthat` strategy only. When the resolved
strategy is the installed-tests fallback, mutator cannot attribute coverage to
test files, so it emits a warning and runs the full suite for every mutant. Pass
`coverage_guided = FALSE` to disable the optimisation (and that warning).
